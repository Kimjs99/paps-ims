...
### 핵심 기능 요약
- **학생 관리**: 양식 가져오기 마법사(임의 양식 CSV/Excel — 헤더 탐지·컬럼 매핑·학번 생성·측정 기록 동시 등록), CSV 템플릿(남/여·M/F, 인코딩 자동 감지), 개별 등록, 비활성화/학급 관리
- **측정**: 학급·개별 입력, 임시저장, 측정 CSV 템플릿, 키/몸무게 student 연동
- **PAPS 공식 산출**: 교육부 기준표 종목별 등급 + 기록→점수(0~20) 총점(100점) 기반 공식 종합등급 — 기록시트지 199건 100% 재현 검증
- **대시보드·보고서**: 필터·차트·개인 조회, 학급 보고서·개인 성장 카드 PDF, Excel 내보내기
- **운영**: 스키마 버전 관리·자동 마이그레이션, 재계산, rowIndex 쓰기 가드, 인증 만료 무손실 복구, 다크 모드, 폰트 자체 호스팅

### ✨ 릴리스 직전 변경
- **학번 자동 생성 규칙 확정**: 학년+반(2자리)+번호(2자리) — 1학년 1반 1번 → `10101` (마법사·템플릿 예시·개별 등록 placeholder 일괄 반영)

### 📝 Docs
- 교사 사용 가이드 갱신 — 양식 가져오기 3단계 등록법, PAPS 공식 등급·점수 체계, FAQ(재계산·인코딩) 추가
- README 정식 릴리스 기준 개편 (주요 기능·문서 링크)

### ✅ 품질
- Vitest **392 테스트**, ESLint 경고 0 기준 유지
...
### ✨ Features
- **학생 가져오기 마법사** — 학생 관리에 "양식 가져오기" 추가. 학교마다 다른 기록시트지(CSV·Excel)를 시스템 템플릿으로 수동 재정리할 필요 없이 직접 가져온다
  - 헤더 행 자동 탐지(제목·날짜 행 건너뜀) + 컬럼 자동 매핑(이름/성명, 성별/남녀, 키/신장, 몸무게/체중 등 동의어 인식), 수동 교정 UI
  - 학년/반 컬럼이 없는 학급 단위 양식은 직접 입력 → 학번을 `학년도+학년+반+번호`로 자동 생성
  - 남/여 변환·중복 학번(파일 내·기존 등록) 차단·키/몸무게 0 미입력 처리 후 기존 일괄 등록 경로로 연결
  - 적용한 매핑을 양식 서명별 프리셋으로 저장 — 같은 양식 재업로드 시 자동 적용
  - XLSX는 동적 로드(기존 지연 청크 재사용), 시트 여러 개면 선택 지원
- 신규 모듈: `pages/app/studentImport.js`(순수 함수) + `components/students/ImportWizardDialog.jsx`

### 🐛 Bug Fixes
- **parseCsv 다중행 셀 파싱** — 따옴표 안 개행(기록시트지의 "셔틀런\n(회)" 형태 헤더)이 행을 중간에 쪼개던 문제. 완전한 따옴표 인식 상태 머신으로 교체 (측정 CSV 업로드 공통 수혜)

### ✅ Tests
- Vitest 346 → **371 케이스**: 헤더 탐지·컬럼 매핑·학번 생성·중복 차단(studentImport), 마법사 컴포넌트 플로우(ImportWizardDialog), parseCsv 다중행 셀 회귀
