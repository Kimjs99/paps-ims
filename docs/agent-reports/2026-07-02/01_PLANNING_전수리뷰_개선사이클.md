...
- [v] #4 라우트 param 변경 시 프리필 가드 미리셋 — `/measure` 라우트에 key 부여로 강제 재마운트 (v0.16.0)
- [v] #5 학교급 변경 재시드 — 설정 학교 정보에 학교급 Select + 확인 다이얼로그 → 재시드 성공 후에만 저장. 시드는 PUT→잔행 clear로 원자성 개선 (v0.16.0)
- [ ] #6 초등 학년 범위 1~6 vs PAPS 기준 3~6 — 명부 관리 목적이면 의도적. 사용자 정책 확인 필요(1~2학년 등록 허용 여부)
- [v] #7 CSV 오류 행 번호 — parseCsvWithLines로 원본 파일 행 번호 보존 (v0.16.0)

## 정책 보류 → 처리 결과 (2026-07-03, v0.14.0)

1. **BMI 등급 성인 기준** — 공식 학년별·성별 기준표를 웹에서 신뢰 가능한 형태로 확보 실패(교재 PDF에 판정표 없음, 블로그는 중·고 학교급 단위 비공식 수치만). 임의 적용 대신 **UI 참고치 표기**(측정 미리보기·보고서 각주)로 처리. → [v] **v0.15.0에서 완결**: 사용자 제공 교육부 PAPS 도움자료(PDF)의 학년별(초4~고3)·성별 판정표 반영, 라이브 재계산 55건 실증.
